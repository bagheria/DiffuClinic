"""
Shared utilities for DiffuClinic inference scripts.
"""
import os
import zipfile
import pandas as pd
...
def load_data_from_zip(zip_path: str) -> pd.DataFrame:
    """Load clinical text + reference summaries from a MultiClinSum zip file.

    Returns DataFrame with columns: Full_Text, Summary
    """
    records = []
...
def find_latest_checkpoint(output_dir: str, output_name: str = "checkpoint") -> tuple[int, list[dict]]:
    """Find the latest checkpoint CSV in output_dir.

    Returns (start_idx, results_list). If no checkpoint found, returns (0, []).
    Checkpoints are scoped by output_name to avoid cross-model contamination.
    """
    ...
    # Sort by the numeric part of checkpoint_{output_name}_N.csv
